/* 
   Mason McIntyre
   04-21-2015
   rscpuecache.java
   The purpose of this program is to represent a relatively simple CPU. This is done by using global variables to simulate registers.
   Accepts 16 bit instructions and is backward compatable with 8 bit instructions and implements the use of a 1K directly mapped cache.
*/

import java.util.*;
import java.io.*;
import java.lang.*;


public class rscpuecache{

  static class Entry{
    byte tag;
    byte[] data = new byte[4];
    
    public Entry(){
      tag = 0;
      for(int i = 0; i < 4; i++)
	data[i] = 0;
    }
    public Entry(byte t, byte[] line){
      tag = t;
      data = line;
    }
  }

  static short AR, PC, AC, R, sTR, sDR, mLine, mNumber;
  static byte DR, IR, TR, M[], mTag, mWord, ret;
  static boolean Z, C, V, N, hit, miss;
  static Entry[] cache = new Entry[1024];
  
  static void fetchCycle(){ /* the fetch cycle */
    int iDR, iPC, iAR, iIR; /* used to print the integer values of DR, PC, and AR */
    AR = PC; /* set Address Register to the Program Counter */
    iAR = (int)(AR & 0xffff); /* conversion made for printing output */
    iPC = (int)(PC & 0xffff); /* conversion made for printing output */
    System.out.printf("fetch 1: AR = %d and PC = %d\n", iAR, iPC);
    //DR = M[iAR]; /* set the Data Register to the Memory allocation stored at index at AR */
    DR = readCache();
    iDR = (int)(DR & 0xff); /* conversion made for printing output */
    PC++; /* increment the Program Counter */
    iPC = (int)(PC & 0xffff); /* conversion made for printing output */
    System.out.printf("fetch 2: DR = %d and PC = %d\n", iDR, iPC);
    /* set the Instruction Register to the value in the Data Register */
    IR = DR; 
    /* set the Address Register to the value stored in the incremented Program Counter */
    AR = PC; 
    iAR = (int)(AR & 0xffff); /* conversion made for printing output */
    iIR = (int)(IR & 0xff);   /* conversion made for printing output */
    System.out.printf("fetch 3: IR = %d and AR = %d\n", iIR, iAR);
  }
  
  static void NOP (){
    /* convert flag booleans (ZCVN) into its binary string representation */
    String flagbits = flagsToString(Z, C, V, N); 
    int iDR, iPC, iAR; /* used to print the integer values of DR, PC, and AR */
    iAR = (int)(AR & 0xffff); /* conversion made for printing output */
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("NOP instruction");
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void LDAC (){
    String flagbits;
    int iPC, iAR;

    System.out.println("LDAC instruction");
        
    iAR = (int)(AR & 0xffff);
    DR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 
    System.out.printf("LDAC1 DR = %d PC = %d AR = %d\n", DR, iPC, iAR);
    
    TR = DR;
    DR = M[iAR];
    PC++;
    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 
    System.out.printf("LDAC2 TR = %d DR = %d PC = %d AR = %d\n", TR, DR, iPC, iAR);

    AR = (short)((TR << 8) | DR);
    iAR = (int)(AR & 0xffff);
    System.out.printf("LDAC3 AR = %d\n", iAR);

    DR = M[iAR];
    System.out.printf("LDAC4 DR = %d\n", DR);

    AC = DR;
    System.out.printf("LDAC5 AC = %d\n", AC);

    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void XLDAC (){
    String flagbits;
    int iPC, iAR;
    
    System.out.println("XLDAC instruction");

    iAR = (int)(AR & 0xffffffff);
    sDR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffffffff);
    iPC = (int)(PC & 0xffffffff); 
    System.out.printf("XLDAC1 DR = %d PC = %d AR = %d\n", sDR, iPC, iAR);
    
    sTR = sDR;
    sDR = M[iAR];
    PC++;
    iAR = (int)(AR & 0xffffffff);
    iPC = (int)(PC & 0xffffffff); 
    System.out.printf("XLDAC2 TR = %d DR = %d PC = %d AR = %d\n", sTR, sDR, iPC, iAR);

    AR = (short)((sTR << 8) | sDR);
    iAR = (int)(AR & 0xffffffff);
    System.out.printf("XLDAC3 AR = %d\n", iAR);

    sDR = M[iAR];
    System.out.printf("XLDAC4 DR = %d\n", sDR);

    sTR = sDR;
    AR++;
    iAR = (int)(AR & 0xffffffff);
    sDR = M[iAR];
    System.out.printf("XLDAC5 DR = %d\n", sDR);

    AC = (short)((sTR << 8) | (sDR & 0x000000ff));
    System.out.printf("XLDAC6 AC = %d\n", AC);

    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, sDR);
  }

  static void STAC (){
    String flagbits; 
    int iPC, iAR;
    
    System.out.println("STAC instruction");
    
    iAR = (int)(AR & 0xffff);
    DR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 
    System.out.printf("STAC1 DR = %d PC = %d AR = %d\n", DR, iPC, iAR);
    
    TR = DR;
    DR = M[iAR];
    PC++;
    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 
    System.out.printf("STAC2 TR = %d DR = %d PC = %d AR = %d\n", TR, DR, iPC, iAR);

    AR = (short)((TR << 8) | DR);
    iAR = (int)(AR & 0xffff);
    System.out.printf("STAC3 AR = %d\n", iAR);

    DR = (byte)AC;
    System.out.printf("STAC4 DR = %d\n", DR);

    writeCache(DR);
    System.out.printf("STAC5 M[AR] = %d\n", M[iAR]);

    iAR = (int)(AR & 0xffff);
    iPC = (int)(PC & 0xffff); 
    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void XSTAC (){
    String flagbits; 
    int iPC, iAR;
    
    System.out.println("XSTAC instruction");
    
    iAR = (int)(AR & 0xffffffff);
    sDR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffffffff);
    iPC = (int)(PC & 0xffffffff); 
    System.out.printf("XSTAC1 DR = %d PC = %d AR = %d\n", sDR, iPC, iAR);
    
    sTR = sDR;
    sDR = M[iAR];
    PC++;
    iAR = (int)(AR & 0xffffffff);
    iPC = (int)(PC & 0xffffffff); 
    System.out.printf("XSTAC2 TR = %d DR = %d PC = %d AR = %d\n", sTR, sDR, iPC, iAR);

    AR = (short)((sTR << 8) | sDR);
    iAR = (int)(AR & 0xffffffff);
    System.out.printf("XSTAC3 AR = %d\n", iAR);

    sDR = AC;
    System.out.printf("XSTAC4 DR = %d\n", sDR);

    writeCache((byte)(sDR >> 8));
    System.out.printf("XSTAC5 M[AR] = %d\n", M[iAR]);

    AR++;
    iAR = (int)(AR & 0xffffffff);
    writeCache((byte)sDR);
    System.out.printf("XSTAC6 M[AR] = %d\n", M[iAR]);

    iAR = (int)(AR & 0xffffffff);
    iPC = (int)(PC & 0xffffffff); 
    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, sDR);
  }

  static void MVAC (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("MVAC instruction");
    R = AC; /* set the general register to the value in the accumulator */

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("MVAC1 R = %d\n", R);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XMVAC (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XMVAC instruction");
    R = AC; /* set the general register to the value in the accumulator */

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("XMVAC1 R = %d\n", R);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void MOVR (){
    String flagbits; 
    int iDR, iPC, iAR; 
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("MOVR instruction");
    AC = R; /* set the accumulator to the value of the general register */

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("MOVR1 AC = %d\n", AC);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XMOVR (){
    String flagbits; 
    int iDR, iPC, iAR; 
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XMOVR instruction");
    AC = R; /* set the accumulator to the value of the general register */

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("XMOVR1 AC = %d\n", AC);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void JUMP (){
    String flagbits;
    int iPC, iAR;

    System.out.println("JUMP instruction");
        
    iAR = (int)(AR & 0xffff);
    DR = M[iAR];
    AR++;
    iAR = (int)(AR & 0xffff);
    System.out.printf("JUMP1 DR = %d AR = %d\n", DR, iAR);
    
    TR = DR;
    DR = M[iAR];
    System.out.printf("JUMP2 TR = %d DR = %d\n", TR, DR);

    PC = (short)((TR << 8) | DR);
    iPC = (int)(PC & 0xffff);
    System.out.printf("JUMP3 PC = %d\n", iPC);

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void JMPZ (){
    String flagbits;
    int iPC = 0, iAR = 0;

    System.out.println("JMPZ instruction");
    
    if(Z == true){
      iAR = (int)(AR & 0xffff);
      DR = M[iAR];
      AR++;
      iAR = (int)(AR & 0xffff);
      System.out.printf("JMPZY1 DR = %d AR = %d\n", DR, iAR);
    
      TR = DR;
      DR = M[iAR];
      System.out.printf("JMPZY2 TR = %d DR = %d\n", TR, DR);

      PC = (short)((TR << 8) | DR);
      iPC = (int)(PC & 0xffff);
      System.out.printf("JMPZY3 PC = %d\n", iPC);
    }
    else{
      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JMPZN1 PC = %d\n", iPC);

      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JMPZN2 PC = %d\n", iPC);
    }

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void JPNZ (){
    String flagbits;
    int iPC = 0, iAR = 0;

    System.out.println("JPNZ instruction");
    
    if(Z == false){
      iAR = (int)(AR & 0xffff);
      DR = M[iAR];
      AR++;
      iAR = (int)(AR & 0xffff);
      System.out.printf("JPNZY1 DR = %d AR = %d\n", DR, iAR);
    
      TR = DR;
      DR = M[iAR];
      System.out.printf("JPNZY2 TR = %d DR = %d\n", TR, DR);

      PC = (short)((TR << 8) | DR);
      iPC = (int)(PC & 0xffff);
      System.out.printf("JPNZY3 PC = %d\n", iPC);
    }
    else{
      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JPNZN1 PC = %d\n", iPC);

      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JPNZN2 PC = %d\n", iPC);
    }

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void JMPC (){
    String flagbits;
    int iPC = 0, iAR = 0;

    System.out.println("JMPC instruction");
    
    if(C == true){
      iAR = (int)(AR & 0xffff);
      DR = M[iAR];
      AR++;
      iAR = (int)(AR & 0xffff);
      System.out.printf("JMPC1 DR = %d AR = %d\n", DR, iAR);
    
      TR = DR;
      DR = M[iAR];
      System.out.printf("JMPCY2 TR = %d DR = %d\n", TR, DR);

      PC = (short)((TR << 8) | DR);
      iPC = (int)(PC & 0xffff);
      System.out.printf("JMPCY3 PC = %d\n", iPC);
    }
    else{
      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JMPCN1 PC = %d\n", iPC);

      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JMPCN2 PC = %d\n", iPC);
    }

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void JV (){
    String flagbits;
    int iPC = 0, iAR = 0;

    System.out.println("JV instruction");
    
    if(V == true){
      iAR = (int)(AR & 0xffff);
      DR = M[iAR];
      AR++;
      iAR = (int)(AR & 0xffff);
      System.out.printf("JVY1 DR = %d AR = %d\n", DR, iAR);
    
      TR = DR;
      DR = M[iAR];
      System.out.printf("JVY2 TR = %d DR = %d\n", TR, DR);

      PC = (short)((TR << 8) | DR);
      iPC = (int)(PC & 0xffff);
      System.out.printf("JVY3 PC = %d\n", iPC);
    }
    else{
      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JVN1 PC = %d\n", iPC);

      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JVN2 PC = %d\n", iPC);
    }

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void JN (){
    String flagbits;
    int iPC = 0, iAR = 0;

    System.out.println("JN instruction");
    
    if(V == true){
      iAR = (int)(AR & 0xffff);
      DR = M[iAR];
      AR++;
      iAR = (int)(AR & 0xffff);
      System.out.printf("JNY1 DR = %d AR = %d\n", DR, iAR);
    
      TR = DR;
      DR = M[iAR];
      System.out.printf("JNY2 TR = %d DR = %d\n", TR, DR);

      PC = (short)((TR << 8) | DR);
      iPC = (int)(PC & 0xffff);
      System.out.printf("JNY3 PC = %d\n", iPC);
    }
    else{
      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JNN1 PC = %d\n", iPC);

      PC++;
      iPC = (int)(PC & 0xffff);
      System.out.printf("JNN2 PC = %d\n", iPC);
    }

    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void ADD (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits;
    int iDR = (int)(DR & 0xff), iPC = (int)(PC & 0xffff), iAR = (int)(AR & 0xffff);
    System.out.println("ADD instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x80); 

    if(AC == 0)
      zcheckb = 1;
 
    ccheck = ((int)((int)AC + (int)R) & 0xffff00);  

    if(ccheck <= 0){
      if(AC < 0 || R < 0)
	ccheck = 1;
      else
	ccheck = 0;
    }

    AC = (byte)(AC + R);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x80); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("ADD1 AC = %d ZCVN = %s\n", AC ,flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XADD (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits;
    int iDR = (int)(DR & 0xff), iPC = (int)(PC & 0xffff), iAR = (int)(AR & 0xffff);
    System.out.println("XADD instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 

    if(AC == 0)
      zcheckb = 1;
 
    ccheck = ((int)((int)AC + (int)R) & 0xffff00);  

    if(ccheck <= 0){
      if(AC < 0 || R < 0)
	ccheck = 1;
      else
	ccheck = 0;
    }

    AC += R;

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XADD1 AC = %d ZCVN = %s\n", AC ,flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void SUB (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits;
    int iDR = (int)(DR & 0xff), iPC = (int)(PC & 0xffff), iAR = (int)(AR & 0xffff);
    System.out.println("SUB instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x80); 

    if(AC == 0)
      zcheckb = 1;

    TR = (byte)R;
    R = (byte)((R ^ 0xff) + 1);

    ccheck = ((int)((int)AC + (int)R) & 0xffff00);  

    if(ccheck <= 0){
      if(AC < 0 || R < 0)
	ccheck = 1;
      else
	ccheck = 0;
    }

    AC = (byte)(AC + R);
    R = TR;

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x80); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("SUB1 AC = %d ZCVN = %s\n", AC ,flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XSUB (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits;
    int iDR = (int)(DR & 0xff), iPC = (int)(PC & 0xffff), iAR = (int)(AR & 0xffff);
    System.out.println("XSUB instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 

    if(AC == 0)
      zcheckb = 1;

    sTR = (short)R;
    R = (short)((R ^ 0xffff) + 1);

    ccheck = ((int)((int)AC + (int)R) & 0xffff00);  

    if(ccheck <= 0){
      if(AC < 0 || R < 0)
	ccheck = 1;
      else
	ccheck = 0;
    }

    AC += R;
    R = sTR;

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XSUB1 AC = %d ZCVN = %s\n", AC ,flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void INAC (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("INAC instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x80); 
    
    if(AC == 0)
      zcheckb = 1;

    if(AC == -1)
      ccheck = 1;

    AC = (byte)(AC + 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();


    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("INAC1 AC = %d ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XINAC (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XINAC instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 
    
    if(AC == 0)
      zcheckb = 1;

    if(AC == -1)
      ccheck = 1;

    AC++;

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();


    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("XINAC1 AC = %d ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void CLAC (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("CLAC instruction");
    /* reset accumulator */
    AC = 0; 
    /* set zero flag */
    Z = true; 
    /* set carry flag */
    C = false;
    /* set overflow flag */ 
    V = false;
    /* set negative flag */
    N = false; 
    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("CLAC1 AC = %d ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XCLAC (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XCLAC instruction");
    /* reset accumulator */
    AC = 0; 
    /* set zero flag */
    Z = true; 
    /* set carry flag */
    C = false;
    /* set overflow flag */ 
    V = false;
    /* set negative flag */
    N = false; 
    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("XCLAC1 AC = %d ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void AND (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("AND instruction");

    AC = (byte)(AC & R);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("ADD1 AC = %d, R = %d ZCVN = %s\n", AC, R, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XAND (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XAND instruction");

    AC = (short)(AC & R);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XADD1 AC = %d, R = %d ZCVN = %s\n", AC, R, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void OR (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("OR instruction");

    AC = (byte)(AC | R);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("OR1 AC = %d, R = %d ZCVN = %s\n", AC, R, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void EXOR (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("EXOR instruction");

    AC = (short)(AC | R);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("EXOR1 AC = %d, R = %d ZCVN = %s\n", AC, R, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XOR (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XOR instruction");

    AC = (byte)(AC ^ R);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XOR1 AC = %d, R = %d ZCVN = %s\n", AC, R, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XXOR (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XXOR instruction");

    AC = (short)(AC ^ R);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XXOR1 AC = %d, R = %d ZCVN = %s\n", AC, R, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void NOT (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("NOT instruction");

    AC = (byte)(AC ^ 0xff);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("NOT1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XNOT (){
    String flagbits;
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XNOT instruction");

    AC = (short)(AC ^ 0xffffffff);

    setZeroFlag();
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XNOT1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void RL (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("RL instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x80); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x80);

    if((AC & 0x80) != 0)
      AC = (byte)((AC << 1) + 1);
    else
      AC = (byte)(AC << 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x80); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("RL1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XRL (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XRL instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x8000);

    if((AC & 0x8000) != 0)
      AC = (short)((AC << 1) + 1);
    else
      AC = (short)(AC << 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XRL1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void RR (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    int rcarry = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("RR instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x80); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x01);

    if((AC & 0x01) != 0)
      AC = (byte)((byte)(AC >>> 1) ^ 0x80);
    else if(AC < 0)
      AC = (byte)((byte)(AC >>> 1) ^ 0x80);
    else
      AC = (byte)(AC >>> 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x80); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("RR1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XRR (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    int rcarry = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XRR instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x01);

    if((AC & 0x01) != 0)
      AC = (short)((short)(AC >>> 1) ^ 0x8000);
    else if(AC < 0)
      AC = (short)((short)(AC >>> 1) ^ 0x8000);
    else
      AC = (short)(AC >>> 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XRR1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void LSL (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("LSL instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x80);

    AC = (byte)(AC << 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x80); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("LSL1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XLSL (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XLSL instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x8000);

    AC = (short)(AC << 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XLSL1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void LSR (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("LSR instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x80); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x01);

    if(AC < 0)
      AC = (byte)((AC >>> 1) ^ 0x80);
    else
      AC = (byte)(AC >>> 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x80); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("LSR1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void XLSR (){
    int ccheck = 0;  /* used to check for a carry in the operation */
    int vcheckb = 0; /* vcheckb: used to record the sign bit before manipulation of AC */ 
    int vchecka = 0; /* vchecka: used to record the sign bit after manipulation of AC */
    int zcheckb = 0, zchecka = 0;
    String flagbits; 
    int iDR, iPC, iAR;
    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    System.out.println("XLSR instruction");

    /* mask out all bits except for the sign bit */
    vcheckb = (int)(AC & 0x8000); 
    
    if(AC == 0)
      zcheckb = 1;

    ccheck = (AC & 0x01);

    if(AC < 0)
      AC = (short)((AC >>> 1) ^ 0x8000);
    else
      AC = (short)(AC >>> 1);

    if(AC == 0)
      zchecka = 1;

    /* mask out all bits except for the sign bit */
    vchecka = (int)(AC & 0x8000); 

    setZeroFlag();
    setCheckFlag(ccheck);
    setOverflowFlag(vcheckb, vchecka, zcheckb, zchecka);
    setNegativeFlag();

    flagbits = flagsToString(Z, C, V, N);
    System.out.printf("XLSR1 AC = %d, ZCVN = %s\n", AC, flagbits);
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void MVI (){
    String flagbits;
    int iPC, iAR;
    iAR = (int)(AR & 0xffff); 
    iPC = (int)(PC & 0xffff);
    System.out.println("MVI instruction");

    DR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffff); 
    iPC = (int)(PC & 0xffff);
    System.out.printf("MVI1 DR = %d PC = %d AR = %d\n", DR, iPC, iAR);

    AC = DR;
    System.out.printf("MVI2 AC = %d\n", AC);

    iAR = (int)(AR & 0xffff); 
    iPC = (int)(PC & 0xffff);
    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void XMVI (){
    String flagbits;
    int iPC, iAR;
    iAR = (int)(AR & 0xffffffff); 
    iPC = (int)(PC & 0xffffffff);
    System.out.println("XMVI instruction");

    DR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffffffff); 
    iPC = (int)(PC & 0xffffffff);
    System.out.printf("XMVI1 DR = %d PC = %d AR = %d\n", sDR, iPC, iAR);

    sTR = sDR;
    DR = M[iAR];
    PC++;
    AR++;
    iAR = (int)(AR & 0xffffffff); 
    iPC = (int)(PC & 0xffffffff);
    System.out.printf("XMVI2 DR = %d PC = %d AR = %d\n", sDR, iPC, iAR);

    AC = (short)((sTR << 8) | sDR);
    System.out.printf("XMVI3 AC = %d\n", AC);

    iAR = (int)(AR & 0xffffffff); 
    iPC = (int)(PC & 0xffffffff);
    flagbits = flagsToString(Z, C, V, N); 
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, DR);
  }

  static void HALT (){
    String flagbits;
    int iDR, iPC, iAR; 

    iAR = (int)(AR & 0xffff);
    iDR = (int)(DR & 0xff); 
    iPC = (int)(PC & 0xffff); 
    flagbits = flagsToString(Z, C, V, N); 
    System.out.println("HALT instruction");
    System.out.printf("Instruction Complete: AC = %d R = %d ZCVN = %s AR = %d PC = %d DR = %d\n", AC, R, flagbits, iAR, iPC, iDR);
  }

  static void setZeroFlag(){
    /* flag set if accumulator is zero */
    if(AC == 0) 
      Z = true;
    else
      Z = false;
  }

  static void setCheckFlag(int ccheck){
    /* if ccheck is a positive integer greater than or equal to 256 set the carry bit */
    if(ccheck != 0) 
      C = true;
    else 
      C = false;
  }

  static void setOverflowFlag(int vcheckb, int vchecka, int zcheckb, int zchecka){
    /* check for sign change in AC, if so setflag set for overflow */
    if((vcheckb != vchecka) && (zcheckb == 0) && (zchecka == 0)) 
      V = true;
    else
      V = false;

    if(vcheckb != vchecka)
      V = true;

  }

  static void setNegativeFlag(){
    /* flag set if the accumulator is negative */
    if(AC < 0) 
      N = true;
    else
      N = false;
  }

  static byte findOpCode (byte op){
    switch(op){
    case (byte)0:   /* 0000 0000 */
      fetchCycle();
      NOP();
      return op;
    case (byte)1:   /* 0000 0001 */
      fetchCycle();
      LDAC();
      return op;
    case (byte)2:   /* 0000 0010 */
      fetchCycle();
      STAC();
      return op;
    case (byte)3:   /* 0000 0011 */
      fetchCycle();
      MVAC();
      return op;
    case (byte)4:   /* 0000 0100 */
      fetchCycle();
      MOVR();
      return op;
    case (byte)5:   /* 0000 0101 */
      fetchCycle();
      JUMP();
      return op;
    case (byte)6:   /* 0000 0110 */
      fetchCycle();
      JMPZ();
      return op;
    case (byte)7:   /* 0000 0111 */
      fetchCycle();
      JPNZ();
      return op;
    case (byte)8:   /* 0000 1000 */
      fetchCycle();
      ADD();
      return op;
    case (byte)9:   /* 0000 1001 */
      fetchCycle();
      SUB();
      return op;
    case (byte)10:   /* 0000 1010 */
      fetchCycle();
      INAC();
      return op;
    case (byte)11:   /* 0000 1011 */
      fetchCycle();
      CLAC();
      return op;
    case (byte)12:   /* 0000 1100 */
      fetchCycle();
      AND();
      return op;
    case (byte)13:   /* 0000 1101 */
      fetchCycle();
      OR();
      return op;
    case (byte)14:   /* 0000 1110 */
      fetchCycle();
      XOR();
      return op;
    case (byte)15:   /* 0000 1111 */
      fetchCycle();
      NOT();
      return op;
    case (byte)16:   /* 0001 0000 */
      fetchCycle();
      JMPC();
      return op;
    case (byte)17:   /* 0001 0001 */
      fetchCycle();
      JV();
      return op;
    case (byte)18:   /* 0001 0010 */
      fetchCycle();
      RL();
      return op;
    case (byte)19:   /* 0001 0011 */
      fetchCycle();
      RR();
      return op;
    case (byte)20:   /* 0001 0100 */
      fetchCycle();
      LSL();
      return op;
    case (byte)21:   /* 0001 0101 */
      fetchCycle();
      LSR();
      return op;
    case (byte)22:   /* 0001 0110 */
      fetchCycle();
      MVI();
      return op;
    case (byte)23:   /* 0001 0111 */
      fetchCycle();
      JN();
      return op;
    case (byte)129:   /* 1000 0001 */
      fetchCycle();
      XLDAC();
      return op;
    case (byte)130:   /* 1000 0010 */
      fetchCycle();
      XSTAC();
      return op;
    case (byte)131:   /* 1000 0011 */
      fetchCycle();
      XMVAC();
      return op;
    case (byte)132:   /* 1000 0100 */
      fetchCycle();
      XMOVR();
      return op;
    case (byte)136:   /* 1000 1000 */
      fetchCycle();
      XADD();
      return op;
    case (byte)137:   /* 1000 1001 */
      fetchCycle();
      XSUB();
      return op;
    case (byte)138:   /* 1000 1010 */
      fetchCycle();
      XINAC();
      return op;
    case (byte)139:   /* 1000 1011 */
      fetchCycle();
      XCLAC();
      return op;
    case (byte)140:   /* 1000 1100 */
      fetchCycle();
      XAND();
      return op;
    case (byte)141:   /* 1000 1101 */
      fetchCycle();
      EXOR();
      return op;
    case (byte)142:   /* 1000 1110 */
      fetchCycle();
      XXOR();
      return op;
    case (byte)143:   /* 1000 1111 */
      fetchCycle();
      XNOT();
      return op;
    case (byte)146:   /* 1001 0010 */
      fetchCycle();
      XRL();
      return op;
    case (byte)147:   /* 1001 0011 */
      fetchCycle();
      XRR();
      return op;
    case (byte)148:   /* 1001 0100 */
      fetchCycle();
      XLSL();
      return op;
    case (byte)149:   /* 1001 0101 */
      fetchCycle();
      XLSR();
      return op;
    case (byte)150:   /* 1001 0110 */
      fetchCycle();
      XMVI();
      return op;
    case (byte)255:  /* 1111 1111 */
      fetchCycle();
      HALT();
      return op;
    default:
      fetchCycle();
      System.out.println("Invalid Intruction!");
      return op;
    }
  }

  static String flagsToString (boolean z, boolean c, boolean v, boolean n){
    StringBuilder stringBuilder = new StringBuilder();
    if(z == true)
      stringBuilder.append("1");
    else 
      stringBuilder.append("0");
    if(c == true)
      stringBuilder.append("1");
    else 
      stringBuilder.append("0");
    if(v == true)
      stringBuilder.append("1");
    else 
      stringBuilder.append("0");
    if(n == true)
      stringBuilder.append("1");
    else 
      stringBuilder.append("0");

    return stringBuilder.toString();
  }

  public static void main(String[] args){
    String filename, line;
    int count = 0, memSize = 65536, iPC = 0;
    byte byteVal;
    File fd;
    Scanner S = new Scanner(System.in);
    byte[] mInit = new byte[4];

    AR = 0;
    AC = 0;
    R = 0;
    PC = 0;
    DR = 0;
    IR = 0;
    TR = 0;
    ret = 0;

    /* initialize data part of cache */
    for(int i = 0; i < 4; i++)
      mInit[i] = 0;
    
    /* initialize cache */
    for(int i = 0; i < 1024; i++){
      cache[i] = new Entry((byte)i, mInit);
    }

    /* allocate array able to hold 2^8 simulated memory allocations */
    M = new byte[memSize]; 

    /* initialize all memory allocations to zero */
    for(int i = 0; i < memSize; i++){ 
      M[i] = 0;
    }
    
    System.out.print("rscpu written by Mason McIntyre\n");
    System.out.print("Enter the name of the file containing the program: \n");
    /* get filename */
    filename = S.next(); 
    
    fd = new File(filename);
    System.out.println("You've opened the file " + filename);

    /* check that the file is valid, if not print error */
    try{ 
      S = new Scanner(fd); 
    }catch (IOException e){
      System.err.println(e);
      System.exit(1);
    }

    /* parse through file line by line */
    while(S.hasNextLine()) { 
      line = S.nextLine();
      /* convert line into integer by parsing the string as a base 16 number and then cast to a byte */
      if(line != null && !line.isEmpty()){
	M[count] = (byte)Integer.parseInt(line, 16); 
	/*
	  Used for debugging input 
	  System.out.printf("%d  %d\n", count, M[count]);
	*/
	count++;
      }
    }

    /* initiate the first fetch instruction */
    System.out.print("End of file reached.\n");

    /* 
	   Used for debugging pruposes, prints initial cache tag and data values.
    for(int i = 0; i < 1024; i++){
      for(int j = 0; j < 4; j++)
	System.out.printf("%d %d%d%d%d\n", cache[i].tag, cache[i].data[0], cache[i].data[1], cache[i].data[2], cache[i].data[3]);
    }
    */
    int iAR;

    while(findOpCode(M[PC]) != (byte)255);
    
  }

  static byte readCache(){
    int iAR;
    mTag = (byte)(AR >> 12);
    mWord = (byte)(AR & 0x0003);
    mLine = (short)((short)((short)((short)(AR << 4)) >>> 6) & 0x03ff);
    mNumber = (short)(AR >> 2);

    System.out.printf("Value of AR = %d\n", AR);
    System.out.printf("The main memory block number = %d\n", mNumber);
    System.out.printf("The cache line number = %d\n", mLine);
    System.out.printf("The cache tag and line = %d:%d%d%d%d\n", cache[mLine].tag, cache[mLine].data[0], cache[mLine].data[1], cache[mLine].data[2], cache[mLine].data[3]);
    
    if(cache[mLine].tag == mTag){
      hit = true;
      miss = false;
      ret = cache[mLine].data[mWord];
      System.out.println("Hit!");
      System.out.printf("The cache tag and line = %d:%d,%d,%d,%d\n", cache[mLine].tag, cache[mLine].data[0], cache[mLine].data[1], cache[mLine].data[2], cache[mLine].data[3]);
    }
    else{
      miss = true;
      hit = false;
      System.out.println("Miss!");
      iAR = (int)(AR & 0x0000ffff);
      ret = M[iAR];
      cache[mLine].tag = mTag;
      for(int i = 0; i < 4; i++){
	cache[mLine].data[i] = M[((AR & 0xfffc) | i)];
      }
      
      System.out.printf("The cache tag and line = %d:%d,%d,%d,%d\n", cache[mLine].tag, cache[mLine].data[0], cache[mLine].data[1], cache[mLine].data[2], cache[mLine].data[3]);
    }
    return ret;
  }

  static void writeCache(byte DR){
    int iAR;
    mTag = (byte)(AR >> 12);
    mWord = (byte)(AR & 0x0003);
    mLine = (short)((short)((short)((short)(AR << 4)) >>> 6) & 0x03ff);
    mNumber = (short)(AR >> 2);

    System.out.printf("Value of AR = %d\n", AR);
    System.out.printf("The main memory block number = %d\n", mNumber);
    System.out.printf("The cache line number = %d\n", mLine);
    System.out.printf("The cache tag and line = %d:%d%d%d%d\n", cache[mLine].tag, cache[mLine].data[0], cache[mLine].data[1], cache[mLine].data[2], cache[mLine].data[3]);
    
    if(cache[mLine].tag == mTag){
      hit = true;
      miss = false;
      iAR = (int)(AR & 0x0000ffff);
      cache[mLine].data[mWord] = DR;
      M[iAR] = DR;
      System.out.println("Hit!");
      System.out.printf("The cache tag and line = %d:%d,%d,%d,%d\n", cache[mLine].tag, cache[mLine].data[0], cache[mLine].data[1], cache[mLine].data[2], cache[mLine].data[3]);
    }
    else{
      miss = true;
      hit = false;
      System.out.println("Miss!");
      iAR = (int)(AR & 0x0000ffff);
      cache[mLine].data[mWord] = DR;
      M[iAR] = DR;
      cache[mLine].tag = mTag;
      for(int i = 0; i < 4; i++){
	cache[mLine].data[i] = M[((AR & 0xfffc) | i)];
      }
      
      System.out.printf("The cache tag and line = %d:%d,%d,%d,%d\n", cache[mLine].tag, cache[mLine].data[0], cache[mLine].data[1], cache[mLine].data[2], cache[mLine].data[3]);
    }
  }

}
